...
(function (global) {
    ...
    var HTMLParser = global.HTMLParser = function (html, handler) {
        var index, chars, match, stack = [], last = html, prevTag, nextTag, stacked;
        stack.last = function () {
        ...
        while (html) {
            ...
            // if (!stack.last() || !special[ stack.last().name ]) {
			stacked = stack.last();
            if (!stacked || (stacked.name.toLowerCase() != 'style' && 
				// skip only script, which has javascript: non-javascript script evaluates as true here
				!(stacked.name.toLowerCase() == 'script'
					&& ((!stacked.attrs['language'] && !stacked.attrs['type'])
						|| (stacked.attrs['language'] && stacked.attrs['language'].toLowerCase() == 'javascript')
						|| (stacked.attrs['type'] && stacked.attrs['type'].toLowerCase() == 'text/javascript'))))
			   ) {
                ...
                if (html.indexOf("<!--") === 0) {
                    ...
                    if (match) {
						// if(html.substr(match[0].length-2,2) === "/>")
                        html = html.substring(match[0].length);
                ...
                stackedTag = stacked.name.toLowerCase(); // stackedTag = stack.last().name.toLowerCase();
                ...
                html = html.replace(reStackedTag, function (all, text) {
                    ...
                    if (stackedTag == 'script'
						// NOTE: ensure, that script contains javascript
						// && ((!stacked.attrs['language'] && !stacked.attrs['type'])
						// 	|| (stacked.attrs['language'] && stacked.attrs['language'].toLowerCase() == 'javascript')
						// 	|| (stacked.attrs['type'] && stacked.attrs['type'].toLowerCase() == 'text/javascript'))
					   ) {
        ...
        function parseStartTag(tag, tagName, rest, unary) {
            ...
            if (closeSelf[ tagName ] && stack.last().name == tagName) {
                parseEndTag("", tagName);
            }
            ...
            // if (handler.start) {
                ...
                rest.replace(attr, function (match, name) {
					...
					kv_attrs[name.toLowerCase()] = value;
                });

				if (!unary)
					stack.push({ name: tagName, attrs: kv_attrs, unary: unary });
...
(function (global) {
    ...
    function collapseWhitespaceSmart(str, prevTag, nextTag) {
        ...
        var tags = ['a', 'b', 'big', 'button', 'em', 'font', 'i', 'img', 'mark', 's', 'small', 'span', 'strike', 'strong', 'sub', 'sup', 'tt', 'u', 'input', 'label', 'select'];

		var leftCollapse = prevTag && (!(prevTag.substr(0,1) === '/' || empty[prevTag])
									   || !((prevTag.substr(0,1) === '/' && tags.indexOf(prevTag.substr(1)) !== -1)
										   || (empty[prevTag] && tags.indexOf(prevTag) !== -1)));
		var rightCollapse = nextTag && (nextTag.substr(0, 1) === '/' ||
										((nextTag.substr(0, 1) !== '/' && !empty[nextTag]) && tags.indexOf(nextTag) === -1));

		// first collapse whitespaces to one
        if (prevTag && nextTag) {
            // strip non space whitespace then compress spaces to one
            str = str.replace(/[\t\n\r]+/g, ' ').replace(/[ ]+/g, ' ');	// NOTE: \t\n\r browser processes as whitespace

			// if only single space left between tags and either one of tags should maintain single space around, then return single space
			if(str === ' ' && (!leftCollapse || !rightCollapse)) {
			   return str;
		   }
        }
